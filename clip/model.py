...
class ResidualAttentionBlock_RMaPLe(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
                 text_layer=False, i=0):
        super().__init__()
        ...
        self.attn_mask = attn_mask
        # This must be consistent with the config file prompt
        self.compound_prompt_nctx = design_details['rmaple_length']
        if i == 0:
            self.first_layer = True
        else:
            self.first_layer = False
    ...
    def forward(self, inputs):
        ...
        x = x + self.attention(self.ln_1(x))
        x = x + self.mlp(self.ln_2(x))
        layerfeatures.append(x)
        return [x, compound_prompts_deeper, counter, layerfeatures]  # return again as a list, so that nn.seq can work
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            ...
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MaPLe(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        elif current_trainer == 'RMaPLe':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_RMaPLe(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        else:
...
class VisionTransformer_RMaPLe(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
                 design_details):
        super().__init__()
        self.input_resolution = input_resolution
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts):
        ...
        # Again combine the inputs, so nn.sequential can work
        outputs = self.transformer([x, compound_deeper_prompts, 0, [first_layer_features]])  # third argument is counter
        x = outputs[0]
        x = x.permute(1, 0, 2)  # LND -> NLD
        ...
        for layer in outputs[3]:
            ...
            layer = self.ln_post(layer[:, 0, :])
            if self.proj is not None:
                layer = layer @ self.proj
    ...
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "MaPLe":
                ...
                self.visual = VisionTransformer_RMaPLe(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
                    layers=vision_layers,
