...
def load_clip_to_cpu(cfg):
    ...
    design_details = {"trainer": 'RMaPLe',
                      "vision_depth": 0,
                      "language_depth": 0, "vision_ctx": 0,
                      "language_ctx": 0,
                      "rmaple_length": cfg.TRAINER.RMAPLE.N_CTX}
...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts, compound_prompts_deeper_text):
        ...
        combined = [x, compound_prompts_deeper_text, 0, [first_layer_features]]  # third argument is the counter which denotes depth of prompt
        ...
        layers_features = []
        for layer in outputs[3]:
            layer = layer.permute(1, 0, 2)
            layer = self.ln_final(layer).type(self.dtype)
            layer = layer[torch.arange(layer.shape[0]), tokenized_prompts.argmax(dim=-1)] @ self.text_projection
            layers_features.append(layer)

        return x, layers_features
...
class MultiModalPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.RMAPLE.N_CTX
        ctx_init = cfg.TRAINER.RMAPLE.CTX_INIT
        dtype = clip_model.dtype
        ctx_dim = clip_model.ln_final.weight.shape[0]
        clip_imsize = clip_model.visual.input_resolution
        cfg_imsize = cfg.INPUT.SIZE[0]
        # Default is 1, which is compound shallow prompting
        assert cfg.TRAINER.RMAPLE.PROMPT_DEPTH >= 1, "For RMAPLE, PROMPT_DEPTH should be >= 1"
        self.compound_prompts_depth = cfg.TRAINER.RMAPLE.PROMPT_DEPTH  # max=12, but will create 11 such shared prompts
        ...
        print('RMAPLE design: Multi-modal Prompt Learning')
        print(f'Initial context: "{prompt_prefix}"')
        print(f"Number of RMAPLE context words (tokens): {n_ctx}")
        ...
        # Minimum can be 1, which defaults to shallow RMAPLE
        # compound prompts
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.USE_ROBUSTLOSS = cfg.DATASET.USE_ROBUSTLOSS
    ...
    def forward(self, image, label=None):
        ...
        text_features, text_layers_features  = self.text_encoder(prompts, tokenized_prompts, deep_compound_prompts_text)
        image_features, image_layers_features = self.image_encoder(image.type(self.dtype), shared_ctx, deep_compound_prompts_vision)
...
class RMaPLe(TrainerX):
    def check_cfg(self, cfg):
        assert cfg.TRAINER.RMAPLE.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.RMAPLE.PREC == "fp32" or cfg.TRAINER.RMAPLE.PREC == "amp":
        ...
        self.scaler = GradScaler() if cfg.TRAINER.RMAPLE.PREC == "amp" else None
    ...
    def forward_backward(self, batch):
        ...
        prec = self.cfg.TRAINER.RMAPLE.PREC
        if prec == "amp":
            with autocast():
